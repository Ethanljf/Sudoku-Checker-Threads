...
int main(void){
    ...
    //Integer variable to denote whether an error has been encountered or not
    int error = 0;
    ...
    //Parse the input file specified by the user and store values in their 
    ...
    //Prompt the user to enter the desired file name from user input
    printf("Enter the name of the sudoku solution file to be read from\n");
    printf("Specify the file extension in the name e.g. file.txt)\nFILE: ");
    fflush(stdout);

    //Retrieve the file name from user input
    fgets(userInput, 81, stdin);
    fileName = strtok(userInput, " ");
    fileName[strlen(fileName)-1] = '\0';

    //Open the specified file and store the values in the sudoku solution into the global 2D grid array
    ...
    for(int i=0; i<9; i++){
        ...
        if(*check == 0){
            printf("There is an error in row %d\n", i+1);
            error = 1;
        }
        free(data);
    }
    ...
    for(int i=0; i<9; i++){
        ...
        //and exit the program
        if(*check == 0){
            printf("There is an error in column %d\n", i+1);
            error = 1;
        }
        free(data);
    }
    ...
    for(int i=0; i<9; i++){
        ...
        //and exit the program
        if(*check == 0){
            printf("There is an error in sub-grid %d\n", i+1);
            error = 1;
        }
    }

    //If the program has not encountered any errors in the solution input file,
    //free the memory allocated to the check pointer and the data struct and print out a congratulatory message.
    free(check);
    free(data);
    if(error == 0){
       printf("Congratulations! This sudoku puzzle has been solved!\n");
    //If the sudoku solution contains errors, print out a corresponding message.
    } else {
        printf("I'm sorry. This sudoku puzzle has NOT been solved correctly\n");
    }
    return 0;
}
